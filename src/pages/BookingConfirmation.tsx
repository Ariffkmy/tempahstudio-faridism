...
import { Calendar, Clock, MapPin, Mail, Package, Users } from 'lucide-react';
...
const BookingConfirmation = () => {
  ...
  const reference = location.state?.reference || booking?.reference || 'BK-2024-001';
  const packageName = location.state?.packageName || 'Wedding Reception Package';

  // Determine navigation paths based on user type
  const isWedding = booking?.booking_type === 'wedding';

  const studioBookingPath = booking?.studio?.id
    ? (isWedding ? `/wedding-booking?studioId=${booking.studio.id}` : `/book/${booking.studio.id}`)
    : '/book';
        ...
        <div className="container">
          <div className="max-w-xl mx-auto text-center">
            ...
            <h1 className="text-3xl font-bold mb-2 animate-slide-up">
              {isWedding ? "Terima kasih kerana menempah dengan kami" : "Terima kasih kerana membuat tempahan"}
            </h1>
            <p className="text-muted-foreground mb-8 animate-slide-up stagger-1">
              {isWedding
                ? "Tempahan majlis perkahwinan anda telah berjaya dihantar. Kami akan menghubungi anda sebentar lagi."
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {isWedding ? 'Wedding Reception' : (booking?.studio_layout?.name || 'Suite Perdana')}
                      </p>
                    </div>
                  </div>
                  ...
                  {isWedding ? (
                    <>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium">Pakej Terpilih</p>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium">Bilangan Tetamu</p>
                          <p className="text-sm text-muted-foreground">{booking?.number_of_pax || '0'} orang</p>
                        </div>
                      </div>
                    </>
                  ) : (
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center animate-slide-up stagger-3">
              ...
              <Button asChild>
                <Link to={bookAnotherSessionPath}>
                  {isWedding ? "Tempah Lagi" : "Tempah Sesi Lain"}
                </Link>
              </Button>
            </div>
          </div>
